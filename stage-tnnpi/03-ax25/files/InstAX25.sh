...
# Copy this script file in /usr/local/src/ax25/InstAX25.sh
# cd into /usr/local/src/ax25
# and execute command : sudo chmod a+x updAX25.sh
# execute command to run the script : ./InstAX25.sh
...
#  
  echo -n -e "\t  *" 
  echo -e "${Green}\t Creating Makefile(s) to prepare toos compilation ${Reset}"
  ./autogen.sh
